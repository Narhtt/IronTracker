...
| 🟠9 | Pas d'ESLint/Prettier | ✅ Corrigé | Configurés et fonctionnels — `npx eslint .` → **0 erreur, 54 avertissements** (détail ci-dessous) |
| 🟠10 | Zéro test | ✅ Corrigé | 4 fichiers de test, **26 tests, tous verts** (`storage`, `validation`, `indexer`, `utils`) |
...
### Constats complémentaires (non présents dans l'audit v1)

- 🟠 **`AnalyticsView` est importée en dur dans `App.tsx`** (pas de `React.lazy`), alors que toutes les autres vues le sont. Comme cette vue embarque Recharts, cela **fait gonfler le chunk principal à 720 Ko minifié / 205 Ko gzippé** — largement le plus gros morceau du bundle, chargé même par quelqu'un qui ne consulte jamais les stats. C'est un oubli isolé (pas un pattern général), donc une correction très ciblée.
- 🟡 54 avertissements ESLint résiduels (`any` explicites, variables inutilisées) — aucune erreur bloquante, mais un nettoyage progressif capitaliserait sur l'effort déjà fait pour activer `strict`.
- 🟡 Tests unitaires présents mais pas d'intégration continue (GitHub Actions) qui les exécute automatiquement à chaque push — le filet de sécurité existe mais n'est pas encore automatisé.

---

## Partie 2 — Feuille de route priorisée

Deux pistes distinctes comme demandé : **Technique** (fiabilité/perf/stockage) et **Métier** (logiques sport, visualisation, personnalisation). Organisées en phases pour donner un ordre d'exécution clair aux devs. Le social/partage/connexion est explicitement exclu.

### Phase A — Corrections rapides (v4.0.5 - 100% Terminé ✅)

**Technique**

- [x] 1. **Lazy-load `AnalyticsView`** dans `App.tsx`, comme les autres vues. Gain estimé : retire ~150-200 Ko gzip du chargement initial pour qui n'ouvre pas les stats tout de suite. *(✅ Réalisé)*
- [x] 2. **`manualChunks` dans `vite.config.ts`** pour isoler Recharts et html2canvas dans leurs propres chunks (déjà partiellement le cas via le lazy-loading des vues, mais un chunk vendor explicite stabilise le cache navigateur entre déploiements). *(✅ Réalisé)*
- [x] 3. **CI minimale (GitHub Actions)** : un workflow qui lance `lint`, `test`, `build` sur chaque push/PR. Coût faible, évite une régression silencieuse sur `storage.ts` en particulier. *(✅ Réalisé)*
- [x] 4. **Config du bar/plateau modifiable** : `PLATES_AVAILABLE` et le poids de barre (`20kg`) sont configurables dans les Paramètres avec persistence locale et intégration aux calculateurs. *(✅ Réalisé)*
...
### Phase B — Renforcements structurants (v4.0.6-rc.2 ✅ 100% Terminé)

**Technique (Partie 1 — v4.0.6-rc.1 ✅)**
...
- [x] 11. **Export de séance multi-formats (Ticket thermique & Carte sociale façon Strava)** pour un partage et archivage optimal. *(✅ Réalisé)*
  - *Ticket PNG thermique amélioré* : Monospace stylisé, décomposition des séries, jauge d'intensité relative et détails techniques.
...
- [x] 12. **Visualisation de répartition musculaire calendaire** : jauge segmentée multicolore par jour et analyse d'équilibre. *(✅ Réalisé)*
  - Indicateur segmenté par jour calculant le volume de séries par groupe musculaire ou type d'effort.
  - Identification visuelle instantanée des équilibres et fréquences d'entraînement au fil du mois.
...
1. **Phase A en premier**, sans exception : ce sont des correctifs ciblés, peu risqués, qui referment les derniers trous techniques (bundle, config matériel) et améliorent deux fonctionnalités très visibles (1RM, échauffement) à faible effort.
2. **Phase B ensuite**, à traiter par lots indépendants selon la priorité perçue — l'unité kg/lb (7) et les supersets (11) sont probablement les deux items à plus fort impact perçu par des utilisateurs externes.
3. **Phase C** à cadrer seulement une fois qu'un usage réel avec plusieurs personnes aura confirmé quels axes comptent le plus — notamment la migration IndexedDB (15), qui ne se justifie que si le volume de données le demande réellement.

Chaque item de ce rapport est indépendant et livrable séparément, dans l'esprit de la méthode déjà utilisée pour le patch précédent.
